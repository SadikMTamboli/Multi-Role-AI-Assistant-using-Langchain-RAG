...
st.title("Multi-Role AI Assistant")


proj=st.sidebar.selectbox("Select agent",
    ("AI role", "RAG"))

if proj=="AI role":
        
    st.subheader("AI Role Assistant")

    roles=("","Software Engineer","Specialist Doctor","Senior AI Engineer","Interviewer Mode",
        "Teacher Mode","Debugging Assistant","SQL Expert")
    option = st.selectbox(
        "Select the AI role",
        roles
    )
    ...
    prompt=ChatPromptTemplate.from_template(
        """
        You are an intelligent {role} assistant.

    Answer the question clearly and concisely only for mentioned role strictly.
    ...
    chain=prompt|model.llm

    userinput=st.chat_input('Ask question')
    response=chain.invoke({'input':userinput,'role':option})
